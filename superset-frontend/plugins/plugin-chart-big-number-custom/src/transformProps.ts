...
import { getDefaultTooltip } from '../../plugin-chart-echarts/src/utils/tooltip';
import { Refs } from '../../plugin-chart-echarts/src/types';

const formatPercentChange = getNumberFormatter(
  NumberFormats.PERCENT_SIGNED_1_POINT,
);

const formatPercentagePoints = getNumberFormatter(
  NumberFormats.FLOAT_1_POINT,
);
    ...
    if (compareIndex < sortedData.length) {
      ...
      if (compareToValue !== null && compareFromValue !== null) {
        if (isPercentageMetric) {
          const tempFormatter = getValueFormatter(
            metric,
            currencyFormats,
            columnFormats,
            yAxisFormat,
          ...
          const formattedCurrent = tempFormatter(Number(compareToValue));
          const formattedPrevious = tempFormatter(Number(compareFromValue));

          const numCurrent = parseFloat(String(formattedCurrent).replace(/[^0-9.-]/g, ''));
          const numPrevious = parseFloat(String(formattedPrevious).replace(/[^0-9.-]/g, ''));
          percentChange = numCurrent - numPrevious;

          formattedSubheader = `${percentChange >= 0 ? '+' : ''}${formatPercentagePoints(
            percentChange,
          )} п.п. ${compareSuffix}`;
        } else {
