...
export default function transformProps(
  ...
  const convertColor = (color: any): string | null => {
  if (!color) return null;
  if (typeof color === 'string') return color;
  return `rgba(${color.r}, ${color.g}, ${color.b}, ${color.a})`;
  };

  const colorBelowLowString = convertColor(colorBelowLow);
  const colorBetweenString = convertColor(colorBetween);
  const colorAboveHighString = convertColor(colorAboveHigh);
  ...
  // color of bars
  if (seriesType === 'bar') {
    const isValidThreshold = (v: any) =>
      v !== undefined && v !== null && v !== '' && Number.isFinite(Number(v));

    const hasThresholdColors =
      colorBelowLowString &&
      colorBetweenString &&
      colorAboveHighString &&
      isValidThreshold(thresholdLow) &&
      isValidThreshold(thresholdHigh);

    if (hasThresholdColors) {
      const lowBound = Number(thresholdLow);
      const highBound = Number(thresholdHigh);

      series.forEach(s => {
        if ((s as any).type === 'bar' && s.data) {
          s.data = (s.data as any[]).map((item: any) => {
            ...
            if (Array.isArray(item)) {
              dataValue = item;
              value = Number(item[0]);
              if (isNaN(value)) value = Number(item[1]);
              ...
              if (isNaN(value)) value = Number(item.value[1]);
            ...
            if (value === null || isNaN(value)) return item;

            let color: string;
            if (value < lowBound) color = colorBelowLowString!;
            else if (value <= highBound) color = colorBetweenString!;
            else color = colorAboveHighString!;
